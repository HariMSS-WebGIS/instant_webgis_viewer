...
function generateQR() {
  ...
  // Try uploading to litterbox via different CORS proxies to bypass local ISP/firewall/antivirus blocks
  ...
  var proxies = [
    'https://proxy.corsfix.com/?url=' + encodeURIComponent(litterboxUrl),
    'https://corsproxy.io/?url=' + encodeURIComponent(litterboxUrl),
    'https://thingproxy.freeboard.io/fetch/' + litterboxUrl
  ];
  
  var proxyIdx = 0;

  function tryUploadWithProxy() {
    if (proxyIdx < proxies.length) {
      prog.textContent = 'Uploading map (Attempt ' + (proxyIdx + 1) + '/' + proxies.length + ')...';
      var currentProxy = proxies[proxyIdx];
      proxyIdx++;
      
      fetch(currentProxy, {method: 'POST', body: fd1})
      ...
      // Fallback 1: Direct upload to Litterbox (likely will fail with 403, but worth a try)
      prog.textContent = 'Trying direct upload...';
      fetch(litterboxUrl, {method: 'POST', body: fd1})
        .then(function(r){ return r.text(); })
        .then(function(url){
          url = url.trim();
          if(url.indexOf('http') === 0){ showQRCode(url); return; }
        ...
        .catch(function(){
          // Fallback 2: Upload to tmpfiles.org
          prog.textContent = 'Trying backup server...';
          ...
          fetch('https://tmpfiles.org/api/v1/upload', {method: 'POST', body: fd2})
            ...
            .catch(function(){
              // All methods failed - show helpful error instructions
              step2.style.display  = 'none';
              errBox.style.display = 'block';
              
              // Find or create a custom detailed instructions box in the modal
              var modalBody = document.getElementById('qrmodal');
              if (modalBody) {
                var detailEl = document.getElementById('qr-detail-error');
                if (!detailEl) {
                  detailEl = document.createElement('div');
                  detailEl.id = 'qr-detail-error';
                  detailEl.style.cssText = 'background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:12px;color:#991b1b;font-size:.78rem;margin-top:10px;text-align:left;line-height:1.4';
                  errBox.appendChild(detailEl);
                }
                detailEl.innerHTML = '<strong>Upload Blocked by Network:</strong><br>' +
                  'Your internet connection, antivirus (like Kaspersky/Avast), or firewall is blocking anonymous file sharing services.<br><br>' +
                  '<strong>What you can do:</strong><br>' +
                  '1. <strong>Transfer manually:</strong> Since this file is saved on your computer, you can send it directly to your phone via <strong>WhatsApp Web, Email, or Google Drive</strong>.<br>' +
                  '2. <strong>Disable Antivirus/VPN:</strong> Temporarily disable your VPN or antivirus web shield and try again.';
              }
            });
        });
    }
